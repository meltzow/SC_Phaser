...
export default class Start extends Phaser.Scene
{
	...
	displayPlayers(playerInfo: { x: number; y: number; team: string; playerId: any }, spriteKey: string) {
		const player = this.add.sprite(playerInfo.x, playerInfo.y, spriteKey).setOrigin(0.5, 0.5).setDisplaySize(53, 40);
    ...
    create()
    {
		this.socket = ioclient('http://localhost:' + 8082)
		...
		this.socket.on('playerUpdates', (players: { [x: string]: { y: number, playerId: any, rotation: number, x: number } }) => {
			Object.keys(players).forEach((id) => {
