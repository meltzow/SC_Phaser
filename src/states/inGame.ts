...
export default class InGame extends Phaser.State {
    ...
    nextTick() {
        ...
        EntityUtils.entitiesUpdated.keys().forEach((entity) => {
            let compsUpdated = EntityUtils.entitiesUpdated.getValue(entity);
            this.systems.forEach((system: BaseSystem) => {
        ...
        EntityUtils.entitiesCreated.forEach((entityId: number) => {
            let entity = EntityUtils.getEntity(entityId);
            this.systems.forEach((system: BaseSystem) => {
        ...
        for (let idx2 in EntityUtils.entitiesRemoved) {
            let ent2 = EntityUtils.entitiesRemoved[idx2];
            this.systems.forEach((system: BaseSystem) => {
        ...
        for (let idx3 in EntityUtils.entities) {
            let ent3 = EntityUtils.entities[idx3];
            this.systems.forEach((system: BaseSystem) => {
                let founds = EntityUtils.findEntities(system.components[0], system.components[1]);
                if (!founds || founds.length === 0) {
                    return;
